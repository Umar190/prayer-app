...
const state = {
  ...
  prayer:null, hijri:null, calendar:[], method:15, school:0, highLat:'OneSeventh', timingPreset:'recommended',
...
function restore(){
  try{
    ...
    if(state.timingPreset==='recommended'){ state.method=15; state.school=0; state.highLat='OneSeventh'; }
...
const PRAYER_PROFILE_FALLBACK={countryLabel:'Location-based recommendation',method:15,school:0,highLat:'OneSeventh',methodName:'Moonsighting Committee Worldwide (MCW)',schoolName:'Standard (Shafi / Maliki / Hanbali)',highLatName:'One Seventh of the Night'};
...
function applyRecommendedTimingProfile(){
  ...
  state.method=p.method;state.school=p.school;state.highLat=p.highLat;state.shafaq='general';state.timingPreset='recommended';
  syncTimingControls();
  return p;
}
function syncTimingControls(){
  ...
  const rec=$('#timingRecommendedNote');if(rec){rec.innerHTML=state.timingPreset==='recommended'?`<strong>${escapeHtml(info.methodName)}</strong><p>${escapeHtml(info.schoolName)} · ${escapeHtml(info.highLatName)} · ${escapeHtml(info.countryLabel||'your location')}.</p><p>Noor now defaults to the Moonsighting Committee Worldwide (MCW) method with standard Asr. This is a location/season-aware method used by current UK prayer-time services; local mosque timetables can still differ by a few minutes. In other countries Noor selects a locally relevant supported method where available.</p>`:'<strong>Custom timing</strong><p>Use this when you intentionally follow a particular school, authority or trusted local mosque timetable.</p>';}
