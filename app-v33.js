...
async function fetchHijriMonth(year, month){
  ...
  // The Hijri calendar endpoint does not accept the prayer-time query
  // parameters used by prayerUrl(). Calling it through prayerUrl() caused
  // the full-year calendar to fail in production. Use the endpoint directly.
  const url=`https://api.aladhan.com/v1/hijriCalendar/${month}/${year}`;
  const r=await fetch(url,{cache:'no-store'});
  if(!r.ok) throw new Error(`Hijri month ${month} failed (${r.status})`);
...
async function loadHijriPanel(yearOverride){
  ...
  try{
    ...
    try{
      const approxStartYear=Math.floor(621.5700 + currentYear * 0.970224);
      const gMonths=[]; for(let i=-2;i<18;i++){const d=new Date(approxStartYear,i,1);gMonths.push([d.getFullYear(),d.getMonth()+1]);}
      ...
      try{
        ...
        if(raw){ const saved=JSON.parse(raw); if(Array.isArray(saved?.months) && saved.months.every(Boolean)){ renderHijriYear(currentYear,saved.months); return; } }
      }catch{}
      el.innerHTML='<div class="info-card"><strong>Could not load the full Hijri calendar.</strong><br><small>Check your internet connection and tap Retry. Your prayer times and saved calendar data remain available offline.</small><br><button class="mini-action" id="hijriRetry" style="margin-top:12px">Retry</button></div>';
