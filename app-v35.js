...
async function loadHijriPanel(yearOverride){
  ...
  try{
    const cachedRaw=localStorage.getItem(`noor-hijri-year-${currentYear}`);
    if(cachedRaw){
      try{
        const saved=JSON.parse(cachedRaw);
        if(Array.isArray(saved?.months) && saved.months.length===12 && saved.months.every(m=>Array.isArray(m)&&m.length)){
          renderHijriYear(currentYear,saved.months);
          return;
        }
      }catch{}
    }
    // The direct hijriCalendar endpoint is not consistently usable from browsers.
    // Build the Hijri year from the same Gregorian calendar endpoint already used
    // by Noor's prayer schedule. Fetch a bounded Gregorian range in small batches
    // to avoid browser/API rate limits, then group the returned dates by Hijri month.
    const baseYear=approxGregorianYearForHijri(currentYear);
    const start=new Date(baseYear,4,1); // May of the approximate Gregorian start year
    const pairs=[];
    for(let i=0;i<15;i++){
      const d=new Date(start.getFullYear(),start.getMonth()+i,1);
      pairs.push([d.getFullYear(),d.getMonth()+1]);
    }
    const allRows=await fetchCalendarMonthsBatched(pairs,4);
    const filtered=allRows.flat().filter(x=>Number(x?.date?.hijri?.year)===currentYear);
    const months=Array.from({length:12},(_,i)=>filtered.filter(x=>Number(x?.date?.hijri?.month?.number)===i+1));
    if(months.some(m=>m.length<28)) throw new Error('Incomplete Hijri year');
    state.hijriYearCache={...state.hijriYearCache};
    months.forEach((rows,i)=>state.hijriYearCache[`${currentYear}-${i+1}`]=rows);
    try{ localStorage.setItem(`noor-hijri-year-${currentYear}`, JSON.stringify({savedAt:Date.now(),months})); }catch{}
    renderHijriYear(currentYear,months);
  }catch(err){
    // Last-chance recovery from the prayer calendar endpoint for just the
    // months around the current date, then use cached data if available.
    try{
      const raw=localStorage.getItem(`noor-hijri-year-${currentYear}`);
      if(raw){ const saved=JSON.parse(raw); if(Array.isArray(saved?.months) && saved.months.length===12 && saved.months.every(Boolean)){ renderHijriYear(currentYear,saved.months); return; } }
    }catch{}
    el.innerHTML='<div class="info-card"><strong>Could not load the full Hijri calendar.</strong><br><small>Noor could not retrieve enough calendar data right now. Your saved calendar remains available offline.</small><br><button class="mini-action" id="hijriRetry" style="margin-top:12px">Retry</button></div>';
