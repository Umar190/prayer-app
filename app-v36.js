...
async function fetchGregorianCalendarMonth(year, month){
  const key=`g-${year}-${month}`;
  if(state.hijriYearCache?.[key]) return state.hijriYearCache[key];
  if(state.lat==null || state.lon==null) throw new Error('Location is required for the calendar.');
  const url=prayerUrl(`https://api.aladhan.com/v1/calendar/${year}/${pad(month)}`);
  const r=await fetch(url,{cache:'no-store'});
  if(!r.ok) throw new Error(`Calendar ${year}-${month} failed (${r.status})`);
  const d=await r.json(); const rows=d.data||[];
  if(!rows.length) throw new Error(`Calendar ${year}-${month} empty`);
  state.hijriYearCache[key]=rows;
  return rows;
}
async function fetchCalendarMonthsBatched(pairs,batchSize=4){
  const out=[];
  for(let i=0;i<pairs.length;i+=batchSize){
    const batch=pairs.slice(i,i+batchSize);
    const rows=await Promise.all(batch.map(([y,m])=>fetchGregorianCalendarMonth(y,m)));
    out.push(...rows);
  }
  return out;
}
function approxGregorianYearForHijri(year){ return Math.floor(621.5700 + year * 0.970224); }
...
function hijriFormatter(){
  const tz=state.timezone||Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC';
  try{return new Intl.DateTimeFormat('en-US-u-ca-islamic-umalqura',{timeZone:tz,year:'numeric',month:'numeric',day:'numeric',weekday:'long'});}catch{return new Intl.DateTimeFormat('en-US-u-ca-islamic',{timeZone:tz,year:'numeric',month:'numeric',day:'numeric',weekday:'long'});}
}
function hijriPartsFromDate(date, formatter=hijriFormatter()){
  const parts=formatter.formatToParts(date);
  const get=t=>parts.find(p=>p.type===t)?.value;
  return {year:Number(get('year')),month:Number(get('month')),day:Number(get('day')),weekday:get('weekday')||''};
}
function gregDateLabel(date,tz=state.timezone||'UTC'){
  return new Intl.DateTimeFormat('en-GB',{timeZone:tz,day:'2-digit',month:'2-digit',year:'numeric'}).format(date).replaceAll('/','-');
}
function gregWeekday(date,tz=state.timezone||'UTC'){
  return new Intl.DateTimeFormat('en-GB',{timeZone:tz,weekday:'long'}).format(date);
}
function buildLocalHijriYear(year){
  const formatter=hijriFormatter();
  const base=approxGregorianYearForHijri(year);
  const start=new Date(Date.UTC(base-1,0,1,12));
  const end=new Date(Date.UTC(base+2,11,31,12));
  const months=Array.from({length:12},()=>[]);
  for(let d=new Date(start); d<=end; d.setUTCDate(d.getUTCDate()+1)){
    const h=hijriPartsFromDate(d,formatter);
    if(h.year!==year || h.month<1 || h.month>12) continue;
    const gDate=gregDateLabel(d);
    const gDay=Number(gDate.slice(0,2)), gMonth=Number(gDate.slice(3,5)), gYear=Number(gDate.slice(6,10));
    months[h.month-1].push({
      date:{
        gregorian:{date:`${gDay.toString().padStart(2,'0')}-${gMonth.toString().padStart(2,'0')}-${gYear}`,year:gYear,month:gMonth,day:gDay,weekday:{en:gregWeekday(d)}},
        hijri:{year:h.year,month:{number:h.month,en:hijriMonthLabel(h.month)},day:h.day,designation:'AH'}
      },
      local:true
    });
  }
  if(months.some(m=>m.length<28)) throw new Error(`Could not build complete Hijri year ${year}`);
  return months;
}
async function loadHijriPanel(yearOverride){
  ...
  el.innerHTML=`<div class="info-card">Building the full ${currentYear} AH calendar…</div>`;
  try{
    const cachedRaw=localStorage.getItem(`noor-hijri-local-${currentYear}`);
    if(cachedRaw){
      try{const saved=JSON.parse(cachedRaw);if(Array.isArray(saved?.months)&&saved.months.length===12&&saved.months.every(m=>Array.isArray(m)&&m.length>=28)){renderHijriYear(currentYear,saved.months);return;}}catch{}
    }
    const months=buildLocalHijriYear(currentYear);
    try{localStorage.setItem(`noor-hijri-local-${currentYear}`,JSON.stringify({savedAt:Date.now(),months}));}catch{}
    renderHijriYear(currentYear,months);
  }catch(err){
    el.innerHTML=`<div class="info-card"><strong>Could not build the Hijri calendar on this browser.</strong><br><small>Try another modern browser. Noor will not guess Quranic or religious dates when calendar data is unavailable.</small><br><button class="mini-action" id="hijriRetry" style="margin-top:12px">Retry</button></div>`;
    $('#hijriRetry')?.addEventListener('click',()=>loadHijriPanel(currentYear));
  }
}
