...
function updateQiblaUI(){
  ...
  const label=$('#needleLabel'); if(label) label.style.transform=`translate(-50%, -50%) rotate(${-relative}deg)`;
...
async function openTool(tool){
  ...
  if(tool==='zakat') panel.innerHTML=`<div><div class="eyebrow">ZAKAT</div><h3>Zakat estimator</h3><p class="tool-copy">Enter your net zakatable assets and the nisab threshold you follow. The estimate updates as you change any value.</p><div class="calc-grid"><label>Cash / savings (£)<input id="zCash" type="number" min="0" step="0.01" value="0"></label><label>Gold value (£)<input id="zGold" type="number" min="0" step="0.01" value="0"></label><label>Silver value (£)<input id="zSilver" type="number" min="0" step="0.01" value="0"></label><label>Other zakatable assets (£)<input id="zOther" type="number" min="0" step="0.01" value="0"></label><label>Short-term eligible liabilities (£)<input id="zLiab" type="number" min="0" step="0.01" value="0"></label><label>Nisab threshold (£)<input id="zNisab" type="number" min="0" step="0.01" value="0"></label></div><button class="primary-btn" id="calcZakat">Calculate now</button><div id="zakatResult" class="calc-result"></div><p class="source-note"><strong>How nisab works:</strong> changing the nisab changes whether your net assets are above the threshold. If your net assets remain above both thresholds you try, the estimated 2.5% amount does not change; if they fall below a chosen threshold, the estimate becomes £0. Zakat rules differ by asset type, nisab basis and scholarly methodology. Confirm your calculation with a qualified scholar or trusted calculator for your circumstances.</p></div>`;
  ...
  const recalcZakat=()=>{const assets=['zCash','zGold','zSilver','zOther'].reduce((s,id)=>s+Number($('#'+id)?.value||0),0);const liab=Number($('#zLiab')?.value||0);const nisab=Number($('#zNisab')?.value||0);const net=Math.max(0,assets-liab);const due=nisab>0&&net>=nisab?net*0.025:0;const result=$('#zakatResult');if(!result)return;result.textContent=nisab>0?(due?`Estimated zakat: £${due.toFixed(2)} on £${net.toFixed(2)} of net assets (2.5% estimate). Your entered nisab is £${nisab.toFixed(2)}.`:`No zakat estimate due because net assets (£${net.toFixed(2)}) are below the entered nisab (£${nisab.toFixed(2)}).`):`Net zakatable assets: £${net.toFixed(2)}. Enter the nisab threshold you follow to complete the estimate.`;};
  $('#calcZakat')?.addEventListener('click',recalcZakat);
  ['zCash','zGold','zSilver','zOther','zLiab','zNisab'].forEach(id=>{const el=$('#'+id);el?.addEventListener('input',recalcZakat);el?.addEventListener('change',recalcZakat);});
  recalcZakat();
