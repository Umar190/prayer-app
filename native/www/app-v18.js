...
function applyTheme(){ document.documentElement.dataset.theme = state.darkMode ? 'dark' : 'light'; const toggle=$('#darkModeToggle'); if(toggle)toggle.checked=!!state.darkMode; }
...
const state = {
  ...
  subscription:{status:'free',plan:null}, communityName:'', communityPosts:[], communityPostTimes:[], darkMode:false, currency:'GBP'
};
...
function updateQiblaUI(){
  const target=$('#qiblaTarget'), guide=$('#qiblaGuide');
  if(state.qiblaBearing==null){
    ...
    if(target) target.classList.remove('aligned');
  ...
  if(target) target.classList.toggle('aligned', info.aligned);
...
function closeToolPanel(){
  ...
  window.setTimeout(()=>panel.classList.add('hidden'),180);
}
...
async function openTool(tool){
  ...
  const panel=$('#toolPanel');
  if(!panel)return;
  if(!wasTools) window.requestAnimationFrame(()=>document.getElementById('view-tools')?.scrollIntoView({behavior:'smooth',block:'start'}));
...
function openSettings(){
  $('#settingsModal').classList.remove('hidden'); fillCurrencySelects(); applyTheme(); $('#notificationsToggle').checked=!!state.notifications;$('#clockToggle').checked=!!state.use12h;$('#adjustmentInput').value=state.adjustment||0;$('#reminderOffsetInput').value=state.reminderOffset||0;updateNotificationStatus();updateLocationButtons();
}
function closeSettings(){ $('#settingsModal').classList.add('hidden'); }
async function saveSettings(){
  state.notifications=$('#notificationsToggle').checked;state.use12h=$('#clockToggle').checked;state.darkMode=$('#darkModeToggle')?.checked||false;state.currency=$('#currencySelect')?.value||state.currency||'GBP';state.adjustment=clamp(Number($('#adjustmentInput').value||0),-30,30);state.reminderOffset=clamp(Number($('#reminderOffsetInput').value||0),0,60);state.adhanEnabled=$('#adhanEnabledToggle')?.checked!==false;state.adhanAudioUrl=$('#adhanAudioUrl')?.value.trim()||DEFAULT_ADHAN_URL;
...
function bind(){
  restore(); applyTheme(); fillCurrencySelects();
  ...
  $('#settingsBtn').addEventListener('click',openSettings); $('#closeSettings').addEventListener('click',closeSettings); $('#darkModeToggle')?.addEventListener('change',e=>{state.darkMode=e.target.checked;applyTheme();persist();}); $('#currencySelect')?.addEventListener('change',e=>{state.currency=e.target.value;persist();}); $('#closePremium')?.addEventListener('click',closePremium); $('#saveSettings').addEventListener('click',saveSettings); $('#openSettingsFromPrayer').addEventListener('click',openSettings);
  ...
  $('#editionSelect').value=state.quranEdition||'en.sahih'; $('#quranSearch').addEventListener('input',e=>renderSurahs(e.target.value)); $('#quranSearch').addEventListener('keydown',e=>{if(e.key==='Enter')searchQuran();}); $('#editionSelect').addEventListener('change',async()=>{state.quranEdition=$('#editionSelect').value;persist();if(state.openedSurah)await openSurah(state.openedSurah,state.lastRead?.ayah||1);});
