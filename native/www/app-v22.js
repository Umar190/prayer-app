...
const state = {
  ...
  prayer:null, hijri:null, calendar:[], method:3, school:0, highLat:'AngleBased', timingPreset:'recommended',
  ...
  subscription:{status:'free',plan:null}, communityName:'', communityPosts:[], communityPostTimes:[], darkMode:true, currency:'GBP', timingPreset:'recommended'
};
...
function persist(){
  const payload={
    method:state.method,school:state.school,highLat:state.highLat,timingPreset:state.timingPreset||'recommended',adjustment:state.adjustment,use12h:state.use12h,
...
const PRAYER_PROFILE_FALLBACK={countryLabel:'Location-based recommendation',method:3,school:0,highLat:'AngleBased',methodName:'Muslim World League',schoolName:'Standard (Shafi / Maliki / Hanbali)',highLatName:'Angle-based'};
function recommendedProfileForCountry(country=''){
  const c=String(country||'').toLowerCase();
  if(/pakistan|india|bangladesh|afghanistan/.test(c)) return {countryLabel:country,method:1,school:0,highLat:'AngleBased',methodName:'University of Islamic Sciences, Karachi',schoolName:'Standard (Shafi / Maliki / Hanbali)',highLatName:'Angle-based'};
  if(/saudi arabia|saudi/.test(c)) return {countryLabel:country,method:4,school:0,highLat:'AngleBased',methodName:'Umm Al-Qura, Makkah',schoolName:'Standard (Shafi / Maliki / Hanbali)',highLatName:'Angle-based'};
  if(/united arab emirates|uae/.test(c)) return {countryLabel:country,method:16,school:0,highLat:'AngleBased',methodName:'Dubai',schoolName:'Standard (Shafi / Maliki / Hanbali)',highLatName:'Angle-based'};
  if(/qatar/.test(c)) return {countryLabel:country,method:10,school:0,highLat:'AngleBased',methodName:'Qatar',schoolName:'Standard (Shafi / Maliki / Hanbali)',highLatName:'Angle-based'};
  if(/kuwait/.test(c)) return {countryLabel:country,method:9,school:0,highLat:'AngleBased',methodName:'Kuwait',schoolName:'Standard (Shafi / Maliki / Hanbali)',highLatName:'Angle-based'};
  if(/bahrain|oman/.test(c)) return {countryLabel:country,method:8,school:0,highLat:'AngleBased',methodName:'Gulf Region',schoolName:'Standard (Shafi / Maliki / Hanbali)',highLatName:'Angle-based'};
  if(/egypt/.test(c)) return {countryLabel:country,method:5,school:0,highLat:'AngleBased',methodName:'Egyptian General Authority of Survey',schoolName:'Standard (Shafi / Maliki / Hanbali)',highLatName:'Angle-based'};
  if(/turkey/.test(c)) return {countryLabel:country,method:13,school:0,highLat:'AngleBased',methodName:'Diyanet (Turkey)',schoolName:'Standard (Shafi / Maliki / Hanbali)',highLatName:'Angle-based'};
  if(/malaysia/.test(c)) return {countryLabel:country,method:17,school:0,highLat:'AngleBased',methodName:'JAKIM',schoolName:'Standard (Shafi / Maliki / Hanbali)',highLatName:'Angle-based'};
  if(/indonesia/.test(c)) return {countryLabel:country,method:20,school:0,highLat:'AngleBased',methodName:'Kemenag Indonesia',schoolName:'Standard (Shafi / Maliki / Hanbali)',highLatName:'Angle-based'};
  if(/singapore/.test(c)) return {countryLabel:country,method:11,school:0,highLat:'AngleBased',methodName:'Singapore',schoolName:'Standard (Shafi / Maliki / Hanbali)',highLatName:'Angle-based'};
  if(/france/.test(c)) return {countryLabel:country,method:12,school:0,highLat:'AngleBased',methodName:'UOIF (France)',schoolName:'Standard (Shafi / Maliki / Hanbali)',highLatName:'Angle-based'};
  if(/russia/.test(c)) return {countryLabel:country,method:14,school:0,highLat:'AngleBased',methodName:'Spiritual Administration of Muslims of Russia',schoolName:'Standard (Shafi / Maliki / Hanbali)',highLatName:'Angle-based'};
  if(/tunisia/.test(c)) return {countryLabel:country,method:18,school:0,highLat:'AngleBased',methodName:'Tunisia',schoolName:'Standard (Shafi / Maliki / Hanbali)',highLatName:'Angle-based'};
  if(/algeria/.test(c)) return {countryLabel:country,method:19,school:0,highLat:'AngleBased',methodName:'Algeria',schoolName:'Standard (Shafi / Maliki / Hanbali)',highLatName:'Angle-based'};
  if(/morocco/.test(c)) return {countryLabel:country,method:21,school:0,highLat:'AngleBased',methodName:'Morocco',schoolName:'Standard (Shafi / Maliki / Hanbali)',highLatName:'Angle-based'};
  if(/portugal/.test(c)) return {countryLabel:country,method:22,school:0,highLat:'AngleBased',methodName:'Portugal',schoolName:'Standard (Shafi / Maliki / Hanbali)',highLatName:'Angle-based'};
  if(/jordan/.test(c)) return {countryLabel:country,method:23,school:0,highLat:'AngleBased',methodName:'Jordan',schoolName:'Standard (Shafi / Maliki / Hanbali)',highLatName:'Angle-based'};
...
function applyRecommendedTimingProfile(){
  ...
  state.method=p.method;state.school=p.school;state.highLat=p.highLat;state.timingPreset='recommended';
  syncTimingControls();
  return p;
}
function syncTimingControls(){
  const info=timingProfileInfo();
  [['#settingsMethodSelect',state.method],['#settingsSchoolSelect',state.school],['#settingsHighLatSelect',state.highLat]].forEach(([s,v])=>{const el=$(s);if(el)el.value=String(v);});
  ...
  const rec=$('#timingRecommendedNote');if(rec){rec.innerHTML=state.timingPreset==='recommended'?`<strong>${escapeHtml(info.methodName)}</strong><p>${escapeHtml(info.schoolName)} · ${escapeHtml(info.highLatName)} · ${escapeHtml(info.countryLabel||'your location')}.</p><p>For the UK and similar European locations, this follows the common MWL + standard-Asr pattern used by major services. In other countries Noor selects a locally relevant supported method where available.</p>`:'<strong>Custom timing</strong><p>Use this when you intentionally follow a particular school, authority or trusted local mosque timetable.</p>';}
...
function renderFullPrayer(timings){
  ...
  const info=timingProfileInfo(); $('#prayerIntro').textContent=label?`Calculated from ${label} at ${state.lat?.toFixed(5)}, ${state.lon?.toFixed(5)} using ${info.methodName}, ${info.schoolName.toLowerCase()} and ${info.highLatName.toLowerCase()}.`:'Times are calculated from precise coordinates and the selected timing profile.';
}
...
async function saveSettings(){
  ...
  if(state.timingPreset==='recommended') applyRecommendedTimingProfile();
  else {state.method=Number($('#settingsMethodSelect').value);state.school=Number($('#settingsSchoolSelect').value);state.highLat=$('#settingsHighLatSelect').value;}
  ...
  updateNotificationStatus();toast('Settings saved.');
}
...
function bind(){
  restore(); applyTheme(); fillCurrencySelects(); fillMethodSelects(); syncTimingControls();
  ...
  $('#settingsBtn').addEventListener('click',openSettings); $('#themeBtn')?.addEventListener('click',()=>{state.darkMode=!state.darkMode;applyTheme();persist();}); $('#closeSettings').addEventListener('click',closeSettings); $('#darkModeToggle')?.addEventListener('change',e=>{state.darkMode=e.target.checked;applyTheme();persist();}); $('#currencySelect')?.addEventListener('change',e=>{state.currency=e.target.value;persist();}); $('#closePremium')?.addEventListener('click',closePremium); $('#saveSettings').addEventListener('click',saveSettings); $('#openSettingsFromPrayer')?.addEventListener('click',openSettings); $('#openSettingsFromPrayer2')?.addEventListener('click',openSettings); $('#refreshPrayerTimingBtn')?.addEventListener('click',async()=>{if(state.lat!=null){if(state.timingPreset==='recommended')applyRecommendedTimingProfile();await loadPrayerTimes();await loadWeekCalendar();toast('Prayer times recalculated.');}else getPreciseLocation();}); $('#recalculateFromSettings')?.addEventListener('click',async()=>{if(state.timingPreset==='recommended')applyRecommendedTimingProfile();else{state.method=Number($('#settingsMethodSelect').value);state.school=Number($('#settingsSchoolSelect').value);state.highLat=$('#settingsHighLatSelect').value;}syncTimingControls();if(state.lat!=null){await loadPrayerTimes();await loadWeekCalendar();toast('Prayer times recalculated.');}}); $('#timingPresetSelect')?.addEventListener('change',e=>{state.timingPreset=e.target.value;if(state.timingPreset==='recommended')applyRecommendedTimingProfile();else syncTimingControls();}); $('#settingsMethodSelect')?.addEventListener('change',()=>{state.timingPreset='custom';syncTimingControls();}); $('#settingsSchoolSelect')?.addEventListener('change',()=>{state.timingPreset='custom';syncTimingControls();}); $('#settingsHighLatSelect')?.addEventListener('change',()=>{state.timingPreset='custom';syncTimingControls();});
